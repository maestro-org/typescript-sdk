...
export const AddressesApiAxiosParamCreator = function (configuration: Configuration) {
  return {
    ...
    decodeAddress: async (address: string, options: AxiosRequestConfig = {}): Promise<RequestArgs> => {
      ...
      const localVarPath = `/addresses/{address}/decode`.replace(`{${"address"}}`, encodeURIComponent(String(address)));
    ...
    utxosByAddress: async (
      ...
      const localVarPath = `/addresses/{address}/utxos`.replace(`{${"address"}}`, encodeURIComponent(String(address)));
      // use dummy base URL string because the URL constructor only accepts absolute URLs.
      const localVarUrlObj = new URL(localVarPath, DUMMY_BASE_URL);
      let baseOptions = configuration.baseOptions;
